...
HEADER_FILL = PatternFill(start_color="00274C", end_color="00274C", fill_type="solid")
...
def _write_sheet(
    ...
    ws.cell(row=row, column=1, value=analysis_title).font = Font(
        name="Calibri", size=14, bold=True, color="00274C"
    )
    row += 1
...
def create_workbook(title: str = "Analysis Report") -> Workbook:
    ...
    ws.cell(row=1, column=1, value=title).font = Font(
        name="Calibri", size=16, bold=True, color="00274C"
    )
