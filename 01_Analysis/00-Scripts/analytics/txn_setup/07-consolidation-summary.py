...
Examples:
  - "WAL-MART #3893", "WM SUPERCENTER" → "WALMART (ALL LOCATIONS)"
...
# On a Parquet cache HIT the consolidated column is already in combined_df --
# recomputing it (plus the fallback + summary scans below) was several minutes
# of redundant work per warm run. Recompute only when the cache was rebuilt
# (SKIP_COMBINE False), the consolidation rules changed after the cache was
# written (CONSOLIDATION_STALE, from 02-file-config), or the column is
# missing from an old/partial cache.
_consolidation_recomputed = not (
    SKIP_COMBINE
    and not CONSOLIDATION_STALE
    and 'merchant_consolidated' in combined_df.columns
)

if not _consolidation_recomputed:
    print("\nmerchant_consolidated loaded from Parquet cache -- skipping recompute")

if _consolidation_recomputed:
    # Apply universal consolidation function over the DISTINCT merchant
    # strings, then broadcast back across every row (same trick as
    # tag_competitors, #214). standardize_merchant_name depends only on its
    # input string, so this is identical to row-by-row apply -- but a
    # multi-million-row client has only a few hundred thousand distinct
    # merchants, so the ~960-line if-chain runs per unique value instead of
    # per row. Real-NaN merchants never reach the mapping (dropna), so map
    # leaves them NaN and fillna applies the same 'UNKNOWN MERCHANT' the
    # function's pd.isna branch would.
    ...
    # -----------------------------------------------------------------------
    # When merchant_name is genuinely empty (ATM withdrawals, bank fees,
    # internal transfers), the consolidator returns 'UNKNOWN MERCHANT'. On
    # personal accounts this typically runs ~5-10%; on business it can hit
    # 40% (lots of ACH activity without a recognizable merchant). Lumping
    # them all under one bucket distorts the top-merchant charts and the
    # leakage analysis.
    #
    # Re-label these rows based on the transaction_type column (PIN / SIG /
    # ACH / CHK / ATM / FEE / etc.) so the chart shows what those
    # transactions actually are -- ``ATM WITHDRAWAL'' / ``BANK FEE'' /
    # ``ACH TRANSFER'' / etc. -- rather than one giant ``UNKNOWN MERCHANT''
    # bucket. This is purely cosmetic relabeling; downstream tagging
    # (tag_competitors, financial_services) treats the new labels exactly
    # the same as Unknown (they don't match any competitor pattern).
    # -----------------------------------------------------------------------
    ...
    if _n_unknown_before > 0 and 'transaction_type' in combined_df.columns:
        ...
        # Map common transaction_type codes to descriptive labels. Unknown
        # values fall through to the generic ``UNKNOWN MERCHANT'' label so we
        # don't silently invent buckets for codes we haven't seen.
...
# Calculate consolidation impact -- stdout-only reporting (several full-frame
# scans), so it is skipped along with the recompute on a cache HIT.
if _consolidation_recomputed:
