# ===========================================================================
# SEGMENT SPEND LINES: Monthly Spend Trajectory by Segment (Per-Mailer)
# ===========================================================================
# One figure per mailer wave. Each panel = one segment.
# Two lines per panel: Responder vs Non-Responder avg monthly spend.
# X-axis: month offset relative to mail date. Vertical line at mail month.
# Mailer details (mailed, responded, rate) in suptitle.
#
...
if 'segment_cohort_raw' not in dir() or len(segment_cohort_raw) == 0:
    ...
    # Identify spend offset columns and sort
    _spend_cols = sorted(
        [c for c in segment_cohort_raw.columns if c.startswith('m') and ('+' in c or '-' in c)],
        key=lambda c: int(c[1:])
    )
    _offsets = [int(c[1:]) for c in _spend_cols]

    if len(_spend_cols) < 3:
        print("    Not enough spend months for line chart.")
    else:
        _waves = (
        ...
        if len(_all_segs) == 0:
            ...
            for wave in _waves:
                ...
                # Only use offsets that have data for this wave
                _wave_spend_cols = [c for c in _spend_cols if c in _wdata.columns and _wdata[c].notna().any()]
                _wave_offsets = [int(c[1:]) for c in _wave_spend_cols]
                ...
                _suptitle = f"Spend Trajectory: {_wave_label}"
                ...
                for s_idx, seg in enumerate(_all_segs):
                    ...
                    for status, color, lw, marker in [
                        ...
                        if n < 5:
                            continue
                        means = [_grp[c].mean() for c in _wave_spend_cols]
                    ...
                    ax.text(0.3, 0.98, 'Mail', transform=ax.get_xaxis_transform(),
                            fontsize=14, color=_MUTED, va='top')

                    ax.set_title(seg, fontsize=20, fontweight='bold',
                                 color=_SEG_COLORS.get(seg, _DARK), pad=8)
                    ax.set_xlabel('Months from Mail', fontsize=16, fontweight='bold', labelpad=4)
